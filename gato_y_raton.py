...
def es_una_posicion_valida(pos):
    x, y = pos
    return 0 <= x < FILAS and 0 <= y < COLUMNAS #limite de movimiento del tablero
...
def evalucion_estado(estado): #Distancia de Manhattan
    gx, gy = estado ["gato"]
    rx, ry = estado ["raton"]
...
def minimax(estado, profundida, es_maximizar):
    ...
    if es_maximizar:  #RATON 
        mejor = -float("inf") #empieza del peor numero
